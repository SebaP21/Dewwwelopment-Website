...
export const SmoothScrollHero = () => {
	useEffect(() => {
		...
		// Zmieniamy typ argumentu funkcji raf
		const raf = (time: DOMHighResTimeStamp) => {
			lenis.raf(time); // Przekazujemy czas do lenis
			requestAnimationFrame(raf); // Ponownie wywołujemy requestAnimationFrame
		};

		const animationFrameId = requestAnimationFrame(raf); // Zapisujemy identyfikator animacji

		return () => {
			cancelAnimationFrame(animationFrameId); // Anulujemy animację przy odmontowaniu
		};
	}, []);
